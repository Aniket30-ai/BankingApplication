...
//	 @Query()
//	 List<Transaction> findBySenderAccountNumberOrReceivingAccountNumberAndTransactionTimeBetween(Long accountNumber, Long accountNumber2,String startDate, String endDate);
	 

//	    List<Transaction> findBySenderAccountNumber(Long accountNumber);

	    List<Transaction> findBySenderAccountNumberAndTransactionTimeBetween(Long accountNumber, LocalDateTime startDate, LocalDateTime endDate);
	    ...
	    @Query("SELECT COALESCE(SUM(t.debit), 0) FROM Transaction t " +
	            "WHERE t.senderAccountNumber = :senderAccountNumber " +
	            "AND t.transactionType = 'DEBIT' " +
	            "AND t.transactionTime BETWEEN :startOfDay AND :endOfDay")
	     Double getTotalDebitedAmountForSenderBetween(
