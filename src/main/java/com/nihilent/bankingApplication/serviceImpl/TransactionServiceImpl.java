...
//		List<Transaction> findBySenderAccountNumberOrReceivingAccountNumber(Long sender, Long receiver);
		List<Transaction> list = transactionRepository.findBySenderAccountNumberOrReceivingAccountNumber(accountNumber, accountNumber);
...
//		Transaction transaction = bySenderAccountNumber.orElseThrow(()->  new NihilentBankException("Invalid account number"));
        ...
        .filter(tx -> 
            (accountNumber.equals(tx.getSenderAccountNumber()) && "DEBIT".equalsIgnoreCase(tx.getTransactionType())) ||
            (accountNumber.equals(tx.getReceivingAccountNumber()) && "CREDIT".equalsIgnoreCase(tx.getTransactionType()))
        )
...
//		            kafkaProducerService.send(record);
	        ...
	        for (Transaction tnx : txns) {
	            ...
	            IndexRequest indexRequest = new IndexRequest(INDEX)
	                    .id(tnx.getTransactionId())  // This becomes _id
...
//	        }
	...
	private void saveFailedTransactionsToFile(List<String> failedTransactions) throws IOException {
	    ...
	    try (BufferedWriter writer = Files.newBufferedWriter(failedFile, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING)) {
	...
	public List<Transaction> getTransactionsByAccountNumberAndDateRange(Long accountNumber, LocalDateTime startDate, LocalDateTime endDate) {
	    ...
	    if (startDate != null && endDate != null) {
	    	 ...
	    	 List<Transaction> list = transactionRepository.findByAccountNumberAndTransactionTimeBetween(accountNumber, startDate, endDate);
	    		List<Transaction> collect = list.stream()
	    		        .filter(tx -> 
	    		            (accountNumber.equals(tx.getSenderAccountNumber()) && "DEBIT".equalsIgnoreCase(tx.getTransactionType())) ||
	    		            (accountNumber.equals(tx.getReceivingAccountNumber()) && "CREDIT".equalsIgnoreCase(tx.getTransactionType()))
	...
	public String upiFundTransafer(String senderUpiID,String recevierUpiId, Double amount ,String remark) throws NihilentBankException {
		...
		Double totalSentToday = transactionRepository.getTotalDebitedAmountForSenderBetween(accountNumber, startOfDay, endOfDay);
		...
		DigitalBankAccount reciverDigitalBankAccount = reciverUpiId.orElseThrow(()-> new NihilentBankException("Invalid reciver UPI ID"));
